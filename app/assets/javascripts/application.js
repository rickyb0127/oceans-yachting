...
jQuery(document).ready(function($) {
  if ($(window).width() <= 872) {
    var initialScreenSize = "mobile";
  } else {
    var initialScreenSize = "regular";
  }

  var getScreenSize = function (ss) {
    $.ajax({
      type: "POST",
      url: "/get_screen_size",
      data : {screen_size:ss}
  ...
  getScreenSize(initialScreenSize);

  $(window).resize(function() {
    if ($(window).width() <= 872) {
      var screenSize = "mobile";
    } else {
      var screenSize = "regular";
    }
    if (screenSize != initialScreenSize) {
